package com.zyh5games.sdk.channel.youxin;
 ...
 */
@Component("31")
public class YouXinChannel extends BaseChannel {
    private static final Logger log = Logger.getLogger(YouXinChannel.class);

    static Map<String, String> loginExtMap;
    static Map<String, String> loginModelMap;

    YouXinChannel() {
        channelId = ChannelId.H5_YouXin;
        channelName = "游心";
...
//        libUrl.add("https://h5.xy51.com/Public/static/xigusdk/xgh5sdk.js?" + System.currentTimeMillis());
        libUrl.add("https://zyh5games.com/sdk/channel/xgh5sdk(YouXin).js" + "?" + System.currentTimeMillis());
        return libUrl;
    }
    ...
    public JSONObject channelLib(Integer appId) {
        ...
        channelData.put("name", "YouXinH5");

        JSONObject config = configMap.get(appId);
        if (config != null && !config.isEmpty()) {
            JSONObject c = new JSONObject();
            c.put("gameId", config.getString(YouXinConfig.GAME_ID));
    ...
    public boolean channelLogin(Map<String, String[]> map, JSONObject userData) {
        ...
        String loginKey = configMap.get(appId).getString(YouXinConfig.COMMON_KEY);
        Arrays.sort(mustKey);
        // 加密串
    ...
    public boolean channelPayInfo(JSONObject orderData, JSONObject channelOrderNo) {
        ...
        String channelGameId = configMap.get(appId).getString(YouXinConfig.GAME_ID);
        String payKey = configMap.get(appId).getString(YouXinConfig.COMMON_KEY);
    ...
    public JSONObject ajaxGetSignature(Integer appId, JSONObject requestInfo, JSONObject result) {
        ...
        String roleKey = configMap.get(appId).getString(YouXinConfig.COMMON_KEY);
        // 加密字符串
        StringBuilder param = new StringBuilder();
