...
function setLog(str, level) {
    if (QuickSDK.debug == true) {
        if (level == 1) {
            console.log('%cQuickSDK致命错误:' + str, 'color:red');
        } else if (level == 2) {
            console.log('%cQuickSDK警告错误:' + str, 'color:gray');
        } else if (level == 0) {
            console.log('%cQuickSDK运行日志:' + str, 'color:gray');
        } else {
            console.log('%cQuickSDK运行日志:' + str, 'color:gray');
        }
    }
...
var QuickSDK = {
    ...
    login: function (callback) {
        ...
        /*        {
                    ...
                    "channelPlatform": {
                        "libUrl": "https:\/\/h5sdk.quicksdk.net\/static\/lib\/QuickGameH5.js",
                        "playUrl": "http:\/\/19cb4b86.ngrok.io\/h5\/QuickSDKTest\/index.php?token=@172@84@167@222@216@137@164@152@112@109@110@106@166@157@165@164@83@94@90@172@165@156@224@214@197@230@157@84@106@89@109@105@106@102@105@169@164@157@140@162@89@170@164@161@212@215@151@168@83@150@105@107@150@105@208@202@200@220@108@150@145@153@105@98@149@100@103@216@172@158@158@172@111@111@103@103@163@155@215@212@100@84@100@89@155@170@186@215@203@226@166@84@106@171@166@167@150@94@84@226@213@217@213@219@171@138@150@157@145@163@151@210@150@152@153@172@158@171@144@148@134@220@160@147@158@165@153@158@122@150@84@175@150@154@154@167@89@98@87@170@216@214@218@144@107@99@109@110@106@109@162@155@155@173@111@175&channelToken=&rand=1578643747"
                     },
                    "channelData": {
                        "id": "325",
                        "name": "QuickGameH5",
                        "marketos": "1",
                        "channel_code": "301",
                        "keys": "[\"codeName\",\"channelClassName\"]",
                        "models": ["codeName", "channelClassName"]
                    },
                    "channelClientParams": {
                        "client_productCode": "75222447995204654723886569483340"
                     }
                    }
         */
    },
    ...
    pay: function (orderInfo, callback) {
        ...
        //向QuickSDK下单获取QuickSDK订单
        var orderData = JSON.parse(orderInfo);
        var channelToken = getParamsExtQuick('channelToken');
        ...
        orderData.channelToken = channelToken;
        getQuickSDKOrderData(orderData, function (getOrderData) {
            ...
            orderData.quickOrder = getOrderData;
            callChannelPay(orderData);
        });
    },
    ...
    setLogoutNotification: function (sregisterLoginoutBackFuntion) {

    },
    setSwitchAccountNotification: function (sregisterSwtichAccountFunction) {

    },
    setChannelEventNotification: function (sregisterChannelNotifyFunction) {

    },
    setOnResumeNotification: function (sNotify) {

    },
    setOnStopNotification: function (sNotify) {

    },
    //注销
    logout: function (callback) {
        //unLoginCallbackFunction = callback;
        //callPlatformFunction('logout',new Object());
    },
    getChannelType: function () {
        return QuickSDK.channelCode;
    },
    getExtendsVal: function (key, callback) {

    },

    getExtrasConfig: function (key) {
...
var saveChannelParams = '';

function doInit(params, callback) {
    var requestUri = getNowHost();
    requestUri = requestUri.replace(/\&amp;/g, '&');
    try {
    ...
    $.ajax({
        type: "GET",
        url: 'https://qkh5api.quickapi.net/webGame/initApi?' + params,
        dataType: "json",
        success: function (data) {
            //load channel lib
            loadChannelLib(data, function () {
                channelParams = data.channelClientParams; // add by andyhua
                callChannelInit(params + '&quick_channel_data=' + JSON.stringify(data.channelData) + '&quick_channel_token=' + data.channelToken);
                callback();
...
function checkOrderObject(orderInfo) {
    ...
    //检查必须参数
    var mustKey = ['productCode', 'goodsId', 'uid', 'userRoleId', 'userRoleName',
        'userServer', 'userLevel', 'cpOrderNo', 'amount', 'subject',
        'desc', 'callbackUrl', 'extrasParams'];
    for (keyIndex in mustKey) {
        var keyName = mustKey[keyIndex];
        if (!orderJson.hasOwnProperty(keyName)) {
            result.result = false;
    ...
    if (orderJson.productCode != QuickSDK.productCode) {
...
function getQuickSDKOrderData(orderData, callback) {
    ...
    orderData.productCode = QuickSDK.productCode;
    orderData.channelCode = QuickSDK.channelCode;

    if (orderData.channelCode === 0) {
    ...
    $.ajax({
        type: "POST",
        url: 'https://qkh5api.quickapi.net/webGame/ajaxGetOrderNo',
        data: orderData,
        dataType: "json",
        success: function (respData) {
            ...
            rebackObj.channelOrder = respData.data.channelOrderNo;
            return callback(rebackObj);
        },
        error: function () {
            rebackObj.status = false;
...
//    {"status":true,"data":{"orderNo":"30120200110161656661015672","amount":"0.01","channelOrderNo":""},"message":""}
}
...
function autoLogin() {
    ...
    $.ajax({
        type: "GET",
        url: 'https://qkh5api.quickapi.net/webGame/loginApi?' + `params`,
        dataType: "json",
        success: function (data) {
            //callback
...
function doLoginCallback(data) {
    QuickSDK.channelCode = data.userData.channelId;

    channelParams = data.channelClientParams;
    channelCode = data.channelData.channel_code;
    userData = data.userData;

    if (loginCallbackFunction != null) {
        var returnObject = new Object;
        returnObject.data = data.userData;
        returnObject.status = data.userData.uid != '' ? true : false;
        returnObject.message = '';
        loginCallbackFunction(returnObject);
    }
}

function loadChannelLib(data, callback) {
    var libUrl = data.channelPlatform.libUrl;
    var quicklib = "https://qkh5api.quickapi.net/static/lib/Channels/Drivers/" + data.channelData.name + ".js";
    loadAsyncScript(quicklib, function () {
        var channelLib = libUrl;
        if (channelLib == null) {
            callback();
        }
        if (typeof (channelLib) == 'object') {
            ...
            var hasSend = false;
            for (var i in channelLib) {
                var thisLib = channelLib[i];
...
function ajaxUploadGameRole(roleInfo, callback) {
    ...
    roleObject.productCode = QuickSDK.productCode;
    roleObject.channelCode = QuickSDK.channelCode;
    ...
    QuickSDK.dataPut("H5角色Vip" + roleObject.vipLevel, "");
    $.ajax({
        type: "GET",
        url: 'https://qkh5api.quickapi.net/webGame/ajaxUploadGameRoleInfo',
        data: roleObject,
        dataType: "json",
        success: function (respData) {
            if (respData == null || typeof (respData) != 'object') {
...
function callChannelInit(params) {
    var QuickProductCode = params.productCode;
    var QuickProductKeu = params.productKey;
...
function callChannelLogin() {
    ...
    skymoonsLib.login(function (res) {
        ...
        messageObject.func = 'Notify_Quick_Login';
...
function callExtFunc(params) {
    var funName = params.fun;
    switch (funName) {
        case 'checkIDCard':
            skymoonsLib.checkIDCard();
            break;
    }
}
...
function callChannelPay(sOrderInfo) {
    ...
    skymoonsLib.pay(orderInfoJson, function (payObject) {
        var messageObject = new Object();
        messageObject.func = 'Notify_Quick_payStatus';
...
function callChannelLogout() {
    ...
    skymoonsLib.logOut(function (res) {
        var messageObject = new Object();
        messageObject.func = 'Notify_Quick_Logout';
        messageObject.params = returnObject;
        document.getElementById('gameFrame').contentWindow.postMessage(JSON.stringify(messageObject), '*');
    })
}/* Zepto v1.2.0 - zepto event ajax form ie - zeptojs.com/license */
...
function setLog(str, level) {
    if (skymoonsLib.debug != false) {
        if (level == 1) {
            console.log('%cQuickGameSDK致命错误:' + str, 'color:red');
        } else if (level == 0) {
            console.log('%cQuickGameSDK警告错误:' + str, 'color:org');
        } else if (level == 3) {
            console.log('%cQuickGameSDK运行日志:' + str, 'color:gray');
        } else {
            console.log('%cQuickGameSDK运行日志:' + str, 'color:gray');
        }
    }
...
var skymoonsLib = {
    ...
    checkIDCard: function (callback) {
        skyCallbackLoginFunc = callback;
        var messageObject = new Object();
        messageObject.func = 'Event_Channel_checkIDCard';
        window.parent.postMessage(JSON.stringify(messageObject), '*');
    },
}
...
window.addEventListener('message', function (e) {
    ...
    switch (funcName) {
        ...
        case 'Callback_Channel_Pay':
            ...
            break;
        case 'Callback_Channel_checkIDCard':
            skyCallbackLoginFunc(messageObject.params);
            break;
    }
