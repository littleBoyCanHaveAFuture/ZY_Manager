...
function initQuick(data) {
    if (!hasInit) {
        let productCode = quickConfig.ProductCode;
        let productKey = quickConfig.ProductKey;
        if ("undefined" == typeof QuickSDK) {
            let count = 0;
            let poll = function () {
                if ("undefined" == typeof QuickSDK) {
                    if (count < 100) {
                        console.info("QuickSDK is undefined wait = " + Date.now());
                        count++;
                        setTimeout(poll, 100);
                    }
                } else {
                    console.info("QuickSDK is inti success " + Date.now());
                }
            };
            poll();
            // for (let i = 0; i < 2000; i++) {
            //     if ("undefined" == typeof QuickSDK) {
            //         setTimeout(console.info("QuickSDK is undefined wait = " + Date.now()), 100);
            //     } else {
            //         console.info("QuickSDK is inti success " + Date.now());
            //         break;
            //     }
        ...
        if ("undefined" !== typeof QuickSDK) {
            QuickSDK.init(productCode, productKey, true, function () {
                console.info("quick init success");
                hasInit = true;
                if (data.hasOwnProperty("quickData")) {
                    console.info("initQuick zyCallChannelLogin quick ---->doLoginCallback");
                    doLoginCallback(data.quickData);
                } else {
                    console.info("initQuick zyCallChannelLogin quick ---->fail");
                    data.userData.uid = "";
                }

                for (let i = 0; i < 10; i++) {
                    setTimeout(console.info("QuickSDK is init wait = " + Date.now()), 100);
                }
            });
        }
    }
