...
!
function(a, b, c) {
    //var openurl = "//testh55.vlcms.com";
    var openurl = "//h5.xy51.com";
    "use strict";
    ...
    h = {
        ...
        urls: {
            online: {
                ...
                pc2Site:location.protocol + openurl+"/mediawide.php/",
                gcSite: location.protocol + openurl+"/mobile.php/",
            },
            dev: {
    ...
    window.addEventListener("message", function(e) {
        ...
        switch (e.data.event) {
            ...
            case "game:isSubscribeWx:callback":
                ...
                break;
            // 实名认证回调
            case "game:getAuthentication:callback":
                delete e.data.event;
                _this.ongetAuthenticationCallback && _this.ongetAuthenticationCallback.call(null, e.data);
                break;
        }
    });
};
...
function getUrl(source) {
    var url = xgGameUtil.url.gcSite;
    if(typeof source == 'object' && source.sdkloginmodel) {
        var urls = xgGameUtil.url;
        for(var i in urls) {
            if(urls[i].indexOf(source.sdkloginmodel)>-1) {
                url = urls[i];
                break;
            }
        }
 ...
 * @param source
 */
GameXG.prototype.getAuthentication = function(source, callback) {
    this.ongetAuthenticationCallback = callback;
    var user = (typeof source == 'object') ? JSON.stringify(source): source;
    $this = this;
    var data = {
        url: getUrl(source) + "Game/getAuthentication",
        callbackName: "getAuthenticationCallback",
        callback: "callback",
        data: {
            user: user,
        },
...
// 登录
GameXG.prototype.loginSdk = function(data, callback) {
    console.log(data);
    this.onCreateRoleCallback = callback;
    $this = this;
    if(!data.game_appid){
        $this.postMessage({ event: "game:loginSdk",code:0, data: "缺少game_appid" });return;
    }
    if(!data.sdkloginmodel){
        $this.postMessage({ event: "game:loginSdk",code:0, data: "缺少sdkloginmodel" });return;
    }
    if(!data.channelExt){
        $this.postMessage({ event: "game:loginSdk",code:0, data: "缺少channelExt" });return;
    }
    switch (data.sdkloginmodel) {
        ...
        case 'media.php':
            ...
            break;
        case 'mediawide.php':
            siteurl = xgGameUtil.url.pc2Site;
            break;
        default:
            $this.postMessage({ event: "game:loginSdk",code:0, data: data.sdkloginmodel+"--sdkloginmodel参数错误" });return;
            break;
    }
    var d = {
            url: siteurl + "GameUtile/loginSdk",
            callbackName: "loginCallback",
            callback: "callback",
            data: {
                game_appid: data.game_appid,
                channelExt: data.channelExt,
            },
            time: 6e3,
            success: function(a) {
                if(a.code==200){
                ...
                $this.postMessage({ event: "game:loginSdk",code:a.code, data: resdata },"*");
            },
            fail: function(a) {
                console.log(a.message)
            }
                    ...
                    var dd = {
                            ...
                            success: function(a) {
                                if(a.code==200){
                                    ...
                                    resdata = a.msg;
                                    $this.postMessage({ event: "game:pay_real_auth",code:a.code, data: resdata },"*");
                                }else{
                                    resdata = a.msg;
                                }
