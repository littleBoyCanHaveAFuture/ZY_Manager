var d2f = function () {
    ...
    var initMessageListener = function () {
        ...
        window.addEventListener("message", function (ae) {
            var af = ae.data;
            console.log("d2f msg got ", JSON.stringify(af));
            if (af.identify && af.identify == "d1234") {
                switch (af.msg) {
                    case u.GETORDERNO:
                        console.log("GETORDERNO receive");
                        var ad = S(af.data);
                        postMsg(u.RETURNORDERNO, {orderData: ad, amount: af.data.amount, id: af.data.id});
                        break;
        ...
        if (!p) {
            if (!ae) {
                ...
                try {
                    ...
                    V = "//dayplay.pagecp.com/index.php";
                    console.log(V);
                    console.log(err)
                }
    ...
    var charge = function (id, amount, callback) {
        S = callback;
        var ad = S({amount: amount, id: id});
        postMsg(u.RETURNORDERNO, {orderData: ad, amount: amount})
    };
    var pay = function (af, ae, ad) {
        console.log("d2f pay");
        postMsg(u.RECHARGE_PAY, {orderData: ae, amount: af});
        r = ad
    };
    ...
    var getFocusState = function (ae, ad) {
        ...
        var ag = ae.appId;
        g = ad;
        postMsg(u.FOCUS_GETSTATE, {openId: ah, openKey: af, appId: ag})
    };
